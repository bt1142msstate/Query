...
function buildHistoryColumnsMarkup(columns) {
  ...
  return '<div class="history-details-scroll-list">' +
    '<div class="tt-filter-container tt-columns-container">' +
    `<ol class="tt-filter-list tt-columns-list">${buildHistoryColumnItems(safeColumns)}</ol>` +
    '</div>' +
    '</div>';
}
...
function buildHistoryFiltersMarkup(filters, dependencies = getDefaultHistoryDetailsDependencies()) {
  ...
  return `<div class="history-details-scroll-list">${fullMarkup}</div>`;
}
