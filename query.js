...
if (downloadBtn) {
  downloadBtn.addEventListener('click', () => {
    ...
    // Get custom table name from input
    const tableNameInput = document.getElementById('table-name-input');
    ...
    const wsData = [displayedFields]; // Header row
    ...
    // Define the table range (entire data area)
    const range = XLSX.utils.decode_range(ws['!ref']);
    const tableRef = `A1:${XLSX.utils.encode_cell({r: range.e.r, c: range.e.c})}`;
    
    // Create safe table name for Excel (remove special characters)
    const safeTableName = tableName.replace(/[^a-zA-Z0-9_]/g, '_');
    
    // Create proper Excel Table
    ws['!tables'] = [{
      ref: tableRef,
      name: safeTableName,
      headerRowCount: 1,
    ...
    // Set worksheet properties for better Excel table recognition
    ws['!autofilter'] = { ref: tableRef };
    
    // Add worksheet to workbook with custom name
