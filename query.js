...
// === Enhanced drag-and-drop helpers with visual feedback ===
function addDragAndDrop(table){
  ...
  // Add manual hover tracking and show shared trash icon
  headers.forEach(h=>{
  ...
  headers.forEach(th=>{
    th.addEventListener('dragstart', e=>{
      isBubbleDrag = false;   // this is a column drag, not a bubble
    ...
    th.addEventListener('dragend', ()=>{
      ...
      headers.forEach(h=>h.classList.remove('th-hover'));
    ...
    th.addEventListener('dragenter', e=>{
      e.preventDefault();
      // Clear any existing highlight before adding a new one
      table.querySelectorAll('.th-drag-over').forEach(el=>el.classList.remove('th-drag-over'));
      if(!th.classList.contains('th-dragging')){
    ...
    th.addEventListener('drop', e=>{
      e.preventDefault();
      e.stopPropagation();           // keep the outer container from also handling this drop
      const toIndex = parseInt(th.dataset.colIndex, 10);

      // 1️⃣  Column reorder drop (text/plain carries fromIndex if it's numeric)
      const fromIndexStr = e.dataTransfer.getData('text/plain').trim();
      if(/^\d+$/.test(fromIndexStr)){             // numeric → header drag
        ...
        clearDropAnchor();
        return;                                  // stop further bubble handling
      }

      // 2️⃣  Bubble drop – insert new field at this position (before or after, depending on pointer)
  ...
  // --- Allow dropping anywhere inside the column, not just on the header ---
  ...
  bodyCells.forEach(td=>{
    ...
    td.addEventListener('drop', e=>{
      ...
      // — Bubble drop: insert new field at this column position (before or after, depending on pointer)
      const bubbleField = e.dataTransfer.getData('bubble-field');
      if(bubbleField){
        if(!displayedFields.includes(bubbleField)){
      ...
      // — Header reorder drop: numeric fromIndex
