    ...
    case 'library_dashboard':
      return {
        body: JSON.stringify({
          ...
          scope: {
            library: payload.library || 'all',
            library_label: payload.library === 'system:MSU' ? 'Mississippi State University' : 'All MLP libraries',
            item_type: Array.isArray(payload.item_types) && payload.item_types.length > 1 ? 'selection' : payload.item_type || 'all',
            item_types: Array.isArray(payload.item_types) ? payload.item_types : undefined,
            item_type_label: Array.isArray(payload.item_types) && payload.item_types.length > 1
              ? `${payload.item_types.length} selected item types`
              : payload.item_type && payload.item_type !== 'all' ? payload.item_type : 'All item types',
            reporting_period: payload.reporting_period || '365'
          },
          circulation: { checkouts: 880229, renewals: 487605, previous_checkouts: 842110, previous_renewals: 469220, checkout_change: 38119, renewal_change: 18385, checkout_change_rate: 0.045, renewal_change_rate: 0.039, comparison_available: true, comparison_coverage_complete: true, period_label: payload.reporting_period === 'fy:MSU:2027' ? 'FY 2027 to date (Jul 1, 2026–Aug 21, 2026)' : payload.reporting_period === 'cy:2026' ? 'Calendar Year 2026 to date' : 'Recent 365 days', coverage_start: payload.reporting_period === 'fy:MSU:2027' ? '20260701' : payload.reporting_period === 'cy:2026' ? '20260101' : '20250822', coverage_end: '20260821', comparison_start: payload.reporting_period === 'fy:MSU:2027' ? '20250701' : payload.reporting_period === 'cy:2026' ? '20250101' : '20240822', comparison_end: payload.reporting_period === 'fy:MSU:2027' ? '20250821' : payload.reporting_period === 'cy:2026' ? '20250821' : '20250821', renewal_share: 0.356, holds: 121843, holds_per_100_items: 4.3 },
          ...
          system_breakdown: [{ label: 'MSU', branches: 2, items: 2813442, checkouts: 880229, renewals: 487605, patrons: 618420 }],
          library_breakdown: [{ label: 'MSU-MAIN', system: 'MSU', checkouts: 208144 }, { label: 'MSU-MERIDIAN', system: 'MSU', checkouts: 30717 }],
          item_type_breakdown: [{ label: 'BOOK', checkouts: 544810 }, { label: 'EBOOK', checkouts: 94220 }],
          ...
          patron_library_breakdown: [{ label: 'MSU-MAIN', patrons: 146820 }, { label: 'MSU-MERIDIAN', patrons: 31220 }],
