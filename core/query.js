...
// HTTP query execution (run, stop, stream, clear) moved to core/queryExecution.js
...
dom.overlay.addEventListener('click',()=>{ 
  ...
  setTimeout(() => window.BubbleSystem?.safeRenderBubbles(), 0);
});
...
window.QueryStateSubscriptions.subscribe(event => {
  ...
  if (event.changes?.activeFilters) {
    updateCategoryCounts();
    // Don't return — also check whether displayedFields has become empty
  }
