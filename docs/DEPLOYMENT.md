...
For the maintained MLP Sirsi deployment, use the hardware-bound `sirsi:ops:*` workflow documented in [`docs/CLI.md`](CLI.md). It reaches the fixed Operations API directly from the approved Mac; Windows App and the workstation bridge are not prerequisites, including when the Operations API updates itself. Before a self-update, verify the independent recovery endpoint. Prepare and review the guarded operation before explicit execution, then verify deployed hashes and behavior. If the primary endpoint fails after the release, use the independent recovery commands to inspect and roll back that operation. Use the workstation only if both direct HTTPS endpoints are unavailable or the operation is genuinely Windows-only.
