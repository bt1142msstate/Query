...
The production snapshot stores each item scope, patron scope, and reporting-window transaction scope once. Requested dashboard views are materialized from those compact dimensions, so activity-window and cross-filter responses do not duplicate full source records. A private planner section adds exact small-category counts, equi-depth numeric/date histograms, correlated policy combinations, record fan-out ratios, and a bounded MARC/note sample. Query uses those statistics to compare complete valid Sirsi routes and estimate all physical stages; it does not retain patron rows or require comparable prior reports.
