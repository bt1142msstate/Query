...
Result hydration is intentionally tolerant while integrations evolve. `src/core/queryResultParser.js` accepts the legacy `X-Raw-Columns` plus pipe-delimited row stream and standard JSON result payloads such as `{ "columns": [...], "rows": [...] }`, `{ "headers": [...], "results": [...] }`, `{ "fields": [...], "data": [...] }`, or a bare array of row objects. JSON array values are preserved as array-backed multi-value cells; older unit-separator strings are still normalized by shared cell-value helpers so table rendering, split columns, post filters, and Excel export share one representation.
