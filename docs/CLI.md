...
Use `npm run query:plan -- --config query.json` to see an evidence-backed ETA before running a report. Planner v2 compares the complete estimated cost of every valid Sirsi starting route, including selector startup and scan, record fan-out, downstream lookups, MARC/note enrichment, parsing, transfer, and final sorting/serialization. When the installed selector can emit SQL, the backend augments its private statistics with Oracle `EXPLAIN PLAN`; criteria that cannot be represented as selector SQL use the private statistics and sample instead. Comparable prior reports are never required.

The plan returns P50, P80, and P90 durations, confidence, expected candidate/scanned/output rows, expected output bytes, the selected route, alternatives compared, per-stage costs, and plain-language safety-limit warnings. The browser shows an immediate aggregate-based estimate as soon as a form is runnable, then replaces it with the backend plan. **Smart order** is on by default in the results toolbar. Select it to switch to **Off** when the displayed filter order must be preserved; ETA planning remains active, and the preference stays on this browser until it is turned back on. During execution, live stage counters refine the remaining ETA.
