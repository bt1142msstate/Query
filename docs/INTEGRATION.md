...
For private deployments, use [`docs/AUTH.md`](AUTH.md) before exposing real library-system data. The recommended path is a same-origin authenticated API route where existing credentials are handled by the backend, reverse proxy, or identity gateway, not by the static frontend.

For non-browser workflows, the CLI in [`docs/CLI.md`](CLI.md) talks to the same backend contract and uses the same payload/result/export modules as the app. It can establish a separate Keychain-backed session through an already signed-in browser using an explicit loopback/PKCE approval flow, use password-stdin as a fallback, verify identity, list fields, run compatibility checks, inspect status, cancel runs, export saved results, list templates, run query JSON configs, apply local post filters, and export JSONL, JSON, CSV, or XLSX. Its generic authenticated `api` command can invoke the complete backend action surface, including template/category mutation, history updates, OCLC/Hydration operations, and future actions. That gives deployments a scriptable path for interactive and scheduled work without adding a separate API.
