...
The authenticated **Hydration** workspace supports single-record and bulk review. Bulk mode can use the complete current query result view directly, accept pasted identifiers, or import text, CSV, TSV, and `.xlsx` Excel files. Excel has two workflows. **Match existing Symphony records** extracts one selected lookup column. **Spreadsheet metadata to MARC** maps every useful column, including title, creators, ISBN, ISSN, LCCN, OCLC number, edition, publication data, language, format, physical description, and series. The backend uses that row as identity evidence, asks OCLC first, uses an exact Library of Congress fallback when available, and abstains when it cannot establish the manifestation safely. Recommended authoritative matches can be downloaded together as binary MARC (`.mrc`) or MARCXML; unmatched or uncertain rows remain in the Excel review and do not produce invented records. The browser parses the workbook locally and sends only normalized mapped values. If the backend-preferred bibliographic identifier is not displayed in the current table, the client reruns the executed backend filters with that identifier added without changing the visible query. Partial result sets must be rerun to completion before that automatic identifier fetch so the record scope cannot silently broaden. The backend field catalog declares usable identifier roles and priorities; the frontend does not hardcode Symphony field names. Hydration has no client-side record-count cap: it processes the complete deduplicated list as a cancelable queue in bounded batches of up to 25 records per Query request. Every batch is stored server-side in an idempotent saved run. Completed, canceled, and partial failed runs appear in Shared History and reopen from saved results without repeating provider calls. Provider lookups remain sequential inside each batch because OCLC matching and exact Library of Congress MARC retrieval are record-oriented operations. If Query returns a rate-limit response, the queue preserves completed work, shows a retry countdown, and retries the same batch instead of falsely failing the remaining records. Completed rows can be downloaded as a client-generated Excel review workbook containing source spreadsheet metadata, local and external identity evidence, provenance, match status, complete tag inventories, generic changed/local-only/source-only tag summaries, and separate 521/526 counts.

Query and Hydration activity history is shared by all authenticated staff. The history panel defaults to **All staff** and can be narrowed to **My runs** using the creator label; credentials, session material, network metadata, and command details remain redacted.

Hydration's browser interface is open source and intended for adoption by other library systems. Workbook import, candidate review, evidence display, bulk-run controls, review exports, and read-only record generation remain in this repository. Authentication, ILS access, OCLC/provider credentials, run persistence, and catalog writes remain backend responsibilities. See [Hydration Integration](docs/HYDRATION.md) for the compatibility boundary and adoption path.
