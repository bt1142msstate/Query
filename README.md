...
The authenticated **Hydration** workspace supports single-record and bulk review. Bulk mode can use the complete current query result view directly, including active post-filters, or accept pasted values and text, CSV, TSV, or `.xlsx` Excel files. Excel parsing stays in the browser; the user chooses a worksheet and lookup column before the extracted values enter the normal preview and matching queue. If the backend-preferred bibliographic identifier is not displayed in the current table, the client reruns the executed backend filters with that identifier added without changing the visible query. Partial result sets must be rerun to completion before that automatic identifier fetch so the record scope cannot silently broaden. The backend field catalog declares usable identifier roles and priorities; the frontend does not hardcode Symphony field names. Hydration has no client-side record-count cap: it processes the complete deduplicated list as a cancelable queue in bounded batches of up to 25 records per Query request. Every batch is stored server-side in an idempotent saved run. Completed, canceled, and partial failed runs appear in Shared History and reopen from saved results without repeating provider calls. Provider lookups remain sequential inside each batch because OCLC matching and exact Library of Congress MARC retrieval are record-oriented operations. If Query returns a rate-limit response, the queue preserves completed work, shows a retry countdown, and retries the same batch instead of falsely failing the remaining records. Direct and automatically selected matches complete without a candidate prompt; ambiguous local titles remain marked for review and never block the rest of the batch. OCLC WorldCat is the primary source. If OCLC produces no acceptable edition match, an exact Library of Congress record identified by the local MARC 010 LCCN can be used as a fallback after passing the same identity and hydration gates. Completed rows can be downloaded as a client-generated Excel review workbook containing local and external-source identity evidence, provenance, match status, complete tag inventories, generic changed/local-only/source-only tag summaries, and separate 521/526 counts.
