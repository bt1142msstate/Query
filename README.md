...
The authenticated **Hydration** workspace supports single-record and bulk review. Bulk mode can use the current query result view directly, including active post-filters, or accept pasted values and text/CSV/TSV files. If the backend-preferred bibliographic identifier is not displayed in the current table, the client reruns the executed backend filters with that identifier added without changing the visible query. Partial result sets must be rerun to completion before that automatic identifier fetch so the record scope cannot silently broaden. The backend field catalog declares usable identifier roles and priorities; the frontend does not hardcode Symphony field names. Bulk review resolves up to 500 unique catalog keys, item IDs, ISBNs, or titles as an incremental queue. Each input completes independently so a slow or failed OCLC request cannot discard neighboring results or create a burst of Metadata API requests. Direct and automatically selected matches complete without a candidate prompt; ambiguous local titles remain marked for review and never block the rest of the batch. Completed rows can be downloaded as a client-generated Excel review workbook containing local and WorldCat identity evidence, match status, complete tag inventories, generic changed/local-only/WorldCat-only tag summaries, and separate 521/526 counts.
