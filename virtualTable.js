...
async function showExampleTable(fields){
  if(!Array.isArray(fields) || fields.length === 0){
    ...
    if(window.updateQueryJson) window.updateQueryJson();
  ...
  window.displayedFields = uniqueFields;
  ...
  if (container) {
    ...
    try {
      await setupVirtualTable(container, window.displayedFields); // local call
    } catch (error) {
    ...
    headerRow.querySelectorAll('th').forEach((th, index) => {
      ...
      const width = calculatedColumnWidths[field] || 150; // local variable
    ...
    measureRowHeight(table, window.displayedFields); // local call
    
    // Initial render of virtual table
    renderVirtualTable(); // local call
    ...
    document.querySelectorAll('.bubble').forEach(bubbleEl => {
      ...
      } else if(window.displayedFields.includes(field)){
    ...
    headers.forEach(h => {
      h.addEventListener('mouseenter', () => {
        ...
        // use DragDropSystem for these properties
        window.DragDropSystem.dragDropManager.hoverTh = h;
        h.appendChild(window.DragDropSystem.headerTrash);
        window.DragDropSystem.headerTrash.style.display = 'block';
      });
      h.addEventListener('mouseleave', () => {
        h.classList.remove('th-hover');
        window.DragDropSystem.dragDropManager.hoverTh = null;
        if (window.DragDropSystem.headerTrash.parentNode) window.DragDropSystem.headerTrash.parentNode.removeChild(window.DragDropSystem.headerTrash);
      });
    });
...
// Export showExampleTable globally for compatibility
